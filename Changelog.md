...
- 新增 blob URL 防护：强制 reload 和 CORS 错误恢复时均跳过 blob: 协议的视频，避免破坏 MSE 流媒体播放器内部状态。
...
- 优化视频选择优先级，视口可见性是硬性门槛：不可见（滚动出视口或宽/高太小）的视频直接被淘汰，不参与评分。可见视频中优先级排序为：鼠标悬停 > 正在播放（面积翻倍） > 面积最大。即一个可见的、面积小的、暂停中的视频，优先级高于一个不可见的、面积大的、播放中的视频，但若视频都没得到鼠标悬停，面积大但没播放的视频得分依然可能比面积小但正在播放的视频得分高。所有视频都被淘汰后，返回第一个`videoWidth > 0`（有实际内容的视频）。
